...
def parse_odr(path, force=False):
	...
	obj_path = "../odr/"+name+".obj"
	mtl_path = "../odr/"+name+".mtl"
	...
	for shader in shaders:
		...
		mtl_data += "newmtl %s\n" % shader["name"]
		print(shader["textures"])
		if shader["textures"]["diffuse"] is not None:
			mtl_data += "map_Kd %s\n" % shader["textures"]["diffuse"].replace("dds", "png") # |map_Kd|base color texture|diffuse texture| | map_Kd soupdoortrim001\woodwrntrim01.dds

		if shader["textures"]["bump"] is not None:
			mtl_data += "map_bump %s\n" % shader["textures"]["bump"].replace("dds", "png") # |map_Bump|normal texture|normal texture| | map_Bump -bm 0.2 bump.png

		if shader["textures"]["spec"] is not None:
			mtl_data += "map_Ks %s\n" % shader["textures"]["spec"].replace("dds", "png") # map_Ks specular.jpeg
